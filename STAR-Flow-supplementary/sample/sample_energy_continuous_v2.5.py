"""
V2.5 采样: DualChannelDenoiser + 37维EVT条件 (无GAT)
架构: 双通道Transformer, 条件维度=37, 支持EVT引导多级对比
"""
import torch
import pandas as pd
import numpy as np
...
OUTPUT_PATH = f"{RESULTS_FOLDER}/generated_scenarios_v2.5.csv"
...
def load_model(model_path):
    ...
    diffusion.eval()
    print(f"V2.5模型加载成功: step={checkpoint.get('step', 'unknown')}")
    return diffusion
...
if __name__ == '__main__':
    print("=" * 60)
    print("V2.5 采样 — DualChannelTransformer + 37dim EVT + CFG (无GAT)")
    print("=" * 60)
