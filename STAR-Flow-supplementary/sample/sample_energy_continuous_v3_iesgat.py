"""
V3-IESGAT 采样: DualChannelDenoiser + IESGraphAttention + CrossAttentionFusion + DDPM

关键增量 vs V3: STAR(全局汇总) → IESGraphAttention(物理拓扑消息传递)
  - wind→elec, solar→elec, elec↔heat(CHP), elec→cold, heat→cold
  - 仅沿物理能量流传递消息 (vs STAR的全局→广播)
"""
import torch
import pandas as pd
import numpy as np
...
CONDITION_DIM = 37
NUM_SCENARIOS = 100
RESULTS_FOLDER = "./results_energy_continuous_v3_iesgat"
...
OUTPUT_PATH = f"{RESULTS_FOLDER}/generated_scenarios_v3_iesgat.csv"
...
def load_model(model_path):
    ...
    if 'ema' in checkpoint:
        ...
        for k in list(clean_state.keys()):
            if 'temporal_proj.weight' in k and k in diffusion.state_dict():
                ckpt_w, model_w = clean_state[k], diffusion.state_dict()[k]
        ...
        # Fix #5: proper cross-var module compat (STAR ↔ IES-GAT)
        ckpt_cv_keys = [k for k in clean_state if k.startswith('cross_var.')]
        model_cv_keys = [k for k in diffusion.state_dict() if k.startswith('cross_var.')]

        if set(ckpt_cv_keys) != set(model_cv_keys):
            print(f"  Cross-var module mismatch: checkpoint has {len(ckpt_cv_keys)} keys, "
                  f"model expects {len(model_cv_keys)} keys")
            # Remove stale checkpoint keys that don't exist in model
            for k in ckpt_cv_keys:
                if k not in diffusion.state_dict():
                    del clean_state[k]
            # Initialize missing model keys from current model
            for k in model_cv_keys:
    ...
    diffusion.eval()
    print(f"V3-IESGAT模型加载成功: step={checkpoint.get('step', 'unknown')}")
    return diffusion
...
if __name__ == '__main__':
    print("=" * 60)
    print("V3-IESGAT 采样 — IES Graph Attention + CrossAttnGate + DDPM")
    print("=" * 60)
