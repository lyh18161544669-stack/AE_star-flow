"""
V3-FM 采样: DualChannelDenoiser + CrossAttentionFusion + Flow Matching (无GAT)

关键特性:
  - Flow Matching ODE采样: 50步确定性积分 (vs DDPM 500步SDE)
  - 37维条件 (无GAT)
  - CFG cond_scale=2.0, hour embedding, STAR模块
  - 支持多级极端程度对比 (normal/moderate/extreme/severe)
"""
import torch
import pandas as pd
import numpy as np
...
SEQ_LEN = 24
CHANNELS = 5
CONDITION_DIM = 37               # 37维, 无GAT
NUM_SCENARIOS = 100
RESULTS_FOLDER = "./results_energy_continuous_v3_fm"
MODEL_PATH = f"{RESULTS_FOLDER}/model-best.pt"
DATA_PATH = "./源荷数据集.csv"

TEMPLATE_DAY = 181
COND_SCALE = 2.0
NUM_SAMPLE_STEPS = 50             # FM ODE integration steps
MULTI_LEVEL_COMPARE = True
OUTPUT_PATH = f"{RESULTS_FOLDER}/generated_scenarios_v3_fm.csv"
...
def load_model(model_path):
    ...
    if 'ema' in checkpoint:
        ...
        # Hour embedding backward compat
        ...
        # Cross-attn fusion backward compat (old additive checkpoints)
        for k in list(clean_state.keys()):
            if k.startswith('fusion.'):
                if k not in diffusion.state_dict():
                    del clean_state[k]
                    print(f"  Removing stale fusion key: {k}")
        missing_fusion = [k for k in diffusion.state_dict()
                         if k.startswith('fusion.') and k not in clean_state]
        if missing_fusion:
            print(f"  Initializing {len(missing_fusion)} fusion keys randomly")
            for k in missing_fusion:
                clean_state[k] = diffusion.state_dict()[k]
        # Variable pos backward compat
        for k in list(clean_state.keys()):
            if 'variable_pos' in k and k not in diffusion.state_dict():
                del clean_state[k]
        missing_pos = [k for k in diffusion.state_dict()
                      if 'variable_pos' in k and k not in clean_state]
        if missing_pos:
            print(f"  Initializing {len(missing_pos)} variable_pos keys randomly")
            for k in missing_pos:
                clean_state[k] = diffusion.state_dict()[k]
    ...
    diffusion.eval()
    print(f"V3-FM模型加载成功: step={checkpoint.get('step', 'unknown')}")
    return diffusion
...
def generate_at_level(diffusion, dataset, fe, extreme_level, per_variable, num_scenarios, template_day):
    ...
    with torch.no_grad():
        generated = diffusion.sample(batch_size=num_scenarios, class_labels=cond_37,
                                     cond_scale=COND_SCALE, num_steps=NUM_SAMPLE_STEPS,
                                     show_progress=True)
...
if __name__ == '__main__':
    print("=" * 60)
    print("V3-FM 采样 — DualChannel + CrossAttnGate + Flow Matching + 37dim EVT + CFG")
    print("=" * 60)
    ...
    print(f"\n[2/3] 配置: day={TEMPLATE_DAY}, cond_scale={COND_SCALE}, "
          f"dim={CONDITION_DIM}, fm_steps={NUM_SAMPLE_STEPS}")
    os.makedirs(RESULTS_FOLDER, exist_ok=True)

    if MULTI_LEVEL_COMPARE:
        print(f"\n[3/3] 多级极端程度对比生成 (FM ODE 50步)...")
