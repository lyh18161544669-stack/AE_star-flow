"""
V3 采样: DualChannelDenoiser + CrossAttentionFusion + 37维EVT条件 (无GAT)

关键特性:
  - 双通道Transformer + 交叉注意力门控融合 (V2.5→V3升级)
  - 37维条件 (无GAT)
...
OUTPUT_PATH = f"{RESULTS_FOLDER}/generated_scenarios_v3.csv"
...
def load_model(model_path):
        ...
        for k in list(clean_state.keys()):
            if k.startswith('fusion.'):
                if k not in diffusion.state_dict():
                    del clean_state[k]
                    print(f"  Removing stale fusion key (additive→cross_attn_gate): {k}")
        missing_fusion = [k for k in diffusion.state_dict()
                         if k.startswith('fusion.') and k not in clean_state]
        if missing_fusion:
            print(f"  Old checkpoint (additive fusion): initializing {len(missing_fusion)} "
                  f"fusion keys randomly")
    ...
    diffusion.eval()
    print(f"V3模型加载成功: step={checkpoint.get('step', 'unknown')}")
    return diffusion
...
if __name__ == '__main__':
    print("=" * 60)
    print("V3 采样 — DualChannelTransformer + CrossAttnGate + 37dim EVT + CFG (无GAT)")
    print("=" * 60)
