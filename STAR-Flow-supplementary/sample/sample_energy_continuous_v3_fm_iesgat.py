"""
V3-FM-IESGAT 采样: DualChannelDenoiser + IESGraphAttention + CrossAttentionFusion + Flow Matching

融合模型: IES-GAT (物理拓扑) + FM (速度场, 50步ODE) + CrossAttnGate
"""
import torch
import pandas as pd
import numpy as np
...
OUTPUT_PATH = f"{RESULTS_FOLDER}/generated_scenarios_v3_fm_iesgat.csv"
...
def load_model(model_path):
    ...
    if 'ema' in checkpoint:
        ...
        # Fusion backward compat
        for k in list(clean_state.keys()):
            if k.startswith('fusion.') and k not in diffusion.state_dict():
                del clean_state[k]
    ...
    diffusion.eval()
    print(f"V3-FM-IESGAT模型加载成功: step={checkpoint.get('step', 'unknown')}")
    return diffusion
...
if __name__ == '__main__':
    print("=" * 60)
    print("V3-FM-IESGAT 采样 — IES-GAT + Flow Matching + CrossAttnGate")
    print("=" * 60)
