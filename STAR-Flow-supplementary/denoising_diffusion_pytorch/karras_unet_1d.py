...
class KarrasUnet1D(Module):
    ...
    def __init__(
        ...
        *,
        ...
        condition_dim = None,     # if set, use continuous condition vector instead of class labels
        ...
        self.random_or_learned_sinusoidal_cond = True  # required by ContinuousTimeGaussianDiffusion
        ...
        if use_hour_embedding:
            input_channels += 2  # sin/cos hour-of-day
        ...
        if use_positional_encoding:
            self.pos_emb = nn.Parameter(torch.randn(1, dim, seq_len))
        ...
        # condition embedding (continuous vector, replaces class labels when condition_dim is set)

        self.condition_dim = condition_dim
        self.cond_drop_prob = cond_drop_prob

        if exists(condition_dim):
            self.to_condition_emb = nn.Sequential(
                Linear(condition_dim, 4 * dim),
                MPSiLU(),
                Linear(4 * dim, 4 * dim),
                Gain()
            )
            self.add_condition_emb = MPAdd(t = mp_add_emb_t)
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        cond_output = self.forward(x, time, class_labels=class_labels,
                                   cond_drop_prob=0.0, **kwargs)

        if cond_scale == 1.0:
            return cond_output, None

        null_output = self.forward(x, time, class_labels=class_labels,
    ...
    def forward(
        ...
        # P1a: hour-of-day embedding as extra input channels
        if self.use_hour_embedding:
            b, c, n = x.shape
            hours = torch.arange(n, device=x.device).float() / n
        ...
        # class condition / condition vector

        needs_any_condition = self.needs_class_labels or exists(self.condition_dim)
        assert xnor(exists(class_labels), needs_any_condition)
        ...
        if exists(self.condition_dim):
            assert class_labels.shape[-1] == self.condition_dim, \
                f'expected condition dim {self.condition_dim}, got {class_labels.shape[-1]}'
            ...
            if cond_drop_prob > 0 and self.training:
                batch = x.shape[0]
                keep_mask = torch.rand(batch, device=x.device) > cond_drop_prob
                ...
                class_labels = torch.where(
                    keep_mask.unsqueeze(-1), class_labels, null_cond
                )

            condition_emb = self.to_condition_emb(class_labels)

            time_emb = self.add_condition_emb(time_emb, condition_emb)
