"""
V3-FM 训练脚本: DualChannelDenoiser + CrossAttentionFusion + Flow Matching (无GAT)

消融定位: V3(DDPM)→V3-FM(FM) 对比扩散框架的效果
         相同的双通道+交叉注意力架构, 仅替换训练/采样范式
         DDPM噪声预测 → FM向量场预测 (velocity field)

关键特性:
  - OT-VP路径: x_t = (1-t)*x_0 + t*x_1 (直线插值)
  - 50步ODE采样 (vs DDPM 500步SDE)
  - CFG在速度空间插值
"""
import torch
...
if __name__ == '__main__':
    ...
    # V3-FM DualChannel架构 (cross-attn gated fusion, 同V3)
    ...
    model = DualChannelDenoiser(
        ...
        fusion_type='cross_attn_gate',   # 同V3
        time_embed_type='fm',            # FM专用时间嵌入 (t∈[0,1])
    )
    ...
    print(f"V3-FM DualChannelDenoiser (Flow Matching + cross-attn gate, no GAT): {n_params:,} params")
    print(f"条件维度: {CONDITION_DIM}, fusion: cross_attn_gate")
    print(f"训练范式: Flow Matching (OT-VP path, velocity field prediction)")
    ...
    accelerator.print(f'V3-FM 训练完成! Best val_loss={best_val_loss:.4f} at step {best_step}')
