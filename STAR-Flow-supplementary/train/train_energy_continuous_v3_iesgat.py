"""
V3-IESGAT 训练: DualChannelDenoiser + IESGraphAttention + CrossAttentionFusion + DDPM

消融定位: V3(STAR)→V3-IESGAT 对比跨变量信息共享机制
         STAR全局softmax汇总 → IES物理拓扑图注意力
         条件维度不变(37), 融合不变(cross_attn_gate)

IES-GAT: 基于IES物理拓扑的图注意力, 替代STAR
  - 物理边: wind→elec, solar→elec, elec↔heat(CHP), elec→cold, heat→cold
...
if __name__ == '__main__':
    ...
    model = DualChannelDenoiser(
        ...
        cross_var_module='ies_gat',    # IES物理拓扑图注意力 (替代STAR)
        cond_drop_prob=0.1,
        use_hour_embedding=True,
        fusion_type='cross_attn_gate',
        time_embed_type='sinusoidal',  # DDPM log-SNR
    )

    diffusion = ContinuousTimeGaussianDiffusion1D(
        model,
        seq_length=SEQ_LEN, channels=CHANNELS,
    ...
    print(f"V3-IESGAT (IES Graph Attention + CrossAttn + DDPM): {n_params:,} params")
    print(f"跨界变量模块: IESGraphAttention (物理拓扑约束)")
    ...
    dl = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True,
                    pin_memory=True, num_workers=min(8, cpu_count()), persistent_workers=True)
    ...
    accelerator.print(f'V3-IESGAT 训练完成! Best val_loss={best_val_loss:.4f} at step {best_step}')
