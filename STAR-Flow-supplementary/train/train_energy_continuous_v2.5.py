"""
V2.5 训练脚本: DualChannelDenoiser + 37维EVT条件 (无GAT)
架构: 双通道Transformer (同V4), 条件维度=37 (同V2)

消融定位: V2→V2.5 对比纯架构升级 (UNet→DualChannel), 条件维度相同
         V2.5→V4 对比GAT的效果 (37→101维条件)
"""
...
if __name__ == '__main__':
    # ================= 配置 =================
    DATA_PATH = "./源荷数据集.csv"
    RESULTS_FOLDER = "./results_energy_continuous_v2.5"

    DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    SEQ_LEN = 24
    CHANNELS = 5
    CONDITION_DIM = 37               # V2.5: 37维, 无GAT
    BATCH_SIZE = 128
    ...
    # V2.5 DualChannel架构 (同V4, 但条件维度=37)
    ...
    model = DualChannelDenoiser(
        seq_len=SEQ_LEN,
        channels=CHANNELS,
        condition_dim=CONDITION_DIM,       # 37维, 无GAT
        ...
        cross_var_module='star',   # STAR全局汇总 (V2.5基线)
        cond_drop_prob=0.1,
    ...
    print(f"V2.5 DualChannelDenoiser (no GAT): {n_params:,} params")
    ...
    accelerator.print(f'V2.5 训练完成! Best val_loss={best_val_loss:.4f} at step {best_step}')
